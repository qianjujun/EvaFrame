 ...
 * @createTime 2020/1/19 14:39
 * @describe
 */
public class PermissionUtils {
    public static void requestPermission(AppCompatActivity activity, PermissionBean permissionBean, PermissionCallBack callBack){
        PermissionFragment fragment = PermissionFragment.create().setPermissionBean(permissionBean).setPermissionCallBack(callBack);
        activity.getSupportFragmentManager().beginTransaction().add(fragment,"permission").commitAllowingStateLoss();
    }

    public static void requestPermission(AppCompatActivity activity, PermissionBean permissionBean){
        requestPermission(activity,permissionBean,null);
    }
    ...
    static ArrayList<String> getFailPermissions(Context context, List<String> permissions) {
        ...
        for (String permission : permissions) {
            ...
            // 检测安装权限
            if (permission.equals(IPermission.REQUEST_INSTALL_PACKAGES)) {
            ...
            // 检测悬浮窗权限
            if (permission.equals(IPermission.SYSTEM_ALERT_WINDOW)) {
            ...
            // 检测8.0的两个新权限
            if (permission.equals(IPermission.ANSWER_PHONE_CALLS) || permission.equals(IPermission.READ_PHONE_NUMBERS)) {
    ...
    static boolean isRequestDeniedPermission(Activity activity, List<String> failPermissions) {
        for (String permission : failPermissions) {
            ...
            if (permission.equals(IPermission.REQUEST_INSTALL_PACKAGES) || permission.equals(IPermission.SYSTEM_ALERT_WINDOW)) {
                continue;
            }
...
//    }
    ...
    static void checkTargetSdkVersion(Context context, List<String> requestPermissions) {
        // 检查是否包含了8.0的权限
        if (requestPermissions.contains(IPermission.REQUEST_INSTALL_PACKAGES)
                || requestPermissions.contains(IPermission.ANSWER_PHONE_CALLS)
                || requestPermissions.contains(IPermission.READ_PHONE_NUMBERS)) {
